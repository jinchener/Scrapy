...
class Downloader(object):
    ...
    def __init__(self, tree, raw_root='raw', date='1978-2014', dbcodename='hgjd'):
        ...
        self.date = date
        self.dbcode = dbcodenames
        if (self.dbcode == 'hgyd') or (self.dbcode == 'hgjd'):
            self.wds = []
        else:
    ...
    def get_paramsmul(self, valuecode):
        params = {'m': 'QueryData', 'dbcode': self.dbcode,
                  'rowcode': 'zb', 'colcode': 'sj',
                  'wds': self.wds,
                  'dfwds': [{'wdcode': 'zb', 'valuecode': None},
    ...
    def download_multi(self, valuecode, to_json=False):
        url = 'http://data.stats.gov.cn/easyquery.htm'
    ...
    def cachell(self, valuecode, content):
        f = open(self.valuecode_path(valuecode), 'wb')
        f.write('{\"data\":[')
...
class Document(object):
    ...
    def json_to_dataframe(self, dic, origin_code=True):
        ...
        for node in datanodes:
            ...
            rd[(zb, sj)] = node['data']['data'] if node[
                'data']['hasdata'] else np.NaN
        return rd

    def json_to_dataframell(self, dic, origin_code=True):
        ...
        for dic in datalist:
            ...
            for node in datanodes:
                ...
                rd[(zb, sj, reg)] = node['data']['data'] if node[
                    'data']['hasdata'] else np.NaN
                if rd not in rdl:
                    rdl.append(rd)
        return rdl
    ...
    def to_file_all(self, tree, filename='result.csv', encoding='utf8'):
        ...
        for j in range(len(titlel)):
            worksheet.write(self.cellnum, j, titlel[j])
